// zkorage Bonded Access — BA1 anonymous per-requirement bond prover host.
...
// in the escrow lock) + a per-requirement nullifier + a NEW-5 in-guest holder signature (accessor-auth),
// binding the requirement (token, min_amount i128, deadline) into the journal, keeping the identity
// (id_secret/id_trapdoor/both leaf indices) PRIVATE, wraps STARK->Groth16, and emits {seal, image_id,
// journal_digest, journal} for the on-chain bond gate. ed25519+sha256 ONLY (no x25519), so the canonical
// image_id reproduces cross-machine (W8 finding).
use ed25519_dalek::{Signer, SigningKey};
use host::encode_seal;
use methods::{BOND_PREDICATE_ELF, BOND_PREDICATE_ID};
...
/// req_id = sha256(token ‖ min_amount(i128 BE) ‖ deadline(u64 BE)) — must match the bond-gate contract.
...
/// (siblings bottom->top, root). The member tree's empty slot is sha256(0x00‖0^32‖0^32); the qualifying
/// tree's is 0^32 — keep the two empties distinct or the roots diverge from the backend/indexer.
...
fn main() {
    ...
    // Inputs come from a job file when ZKORAGE_JOB is set (13 lines, in this order):
    //   sig_hex / pk_hex / accessor_hex / id_secret_hex / id_trapdoor_hex / context_hex / token_hex /
    //   min_amount (i128) / deadline (u64) / member_siblings_hex (depth*32) / member_leaf_index (u32) /
    //   qual_siblings_hex (depth*32) / qual_leaf_index (u32)
    // — the prover-service path: the backend has signed (NEW-5), set context == req_id, and built BOTH
    // Merkle witnesses. Otherwise a DEMO proof is produced (the host self-signs a holder key with pk ==
    // accessor, sets context == req_id, and builds zero-subtree witnesses at index 0 for both trees).
    let (
        sig,
        pk,
        accessor,
    ...
    // Compute both roots + the nullifier + req_id host-side too (for cross-check / pinning).
    ...
    eprintln!("[bond] member_root = {}", hex::encode(mr));
    eprintln!("[bond] qual_root   = {}", hex::encode(qr));
    eprintln!("[bond] req_id      = {}", hex::encode(rid));
    eprintln!("[bond] nullifier   = {}", hex::encode(nf));
    eprintln!("[bond] accessor    = {}", hex::encode(&accessor));
    ...
    if std::env::var("ZKORAGE_EXEC_ONLY").is_ok() {
        ...
        match exec.execute(env, BOND_PREDICATE_ELF) {
            Ok(session) => {
    ...
    let prove_info = prover
        .prove_with_opts(env, BOND_PREDICATE_ELF, &ProverOpts::groth16())
        .expect("groth16 proving failed");
    eprintln!("STATS: {:#?}", prove_info.stats);
    let receipt = prove_info.receipt;

    receipt
        .verify(BOND_PREDICATE_ID)
    ...
    let image_id_digest: Digest = BOND_PREDICATE_ID.into();
    ...
    let out_path = std::env::var("ZKORAGE_OUT").unwrap_or_else(|_| "bundle_bond.json".to_string());
