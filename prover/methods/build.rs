...
// Guest embedding for the host. EIGHT guests live here:
...
//                             ALL EIGHT env vars must be set together (the single host crate compiles
...
// then build the host with ALL EIGHT:
...
//     ZKORAGE_DOCAUTH_ELF=<…> cargo build --release -p host
fn main() {
        ...
        || docauth.is_some()
    {
        ...
        fs::write(
            ...
            format!("{por_rs}{id_rs}{comp_rs}{pay_rs}{acc_rs}{ds_rs}{mem_rs}{da_rs}"),
        )
        .unwrap();
