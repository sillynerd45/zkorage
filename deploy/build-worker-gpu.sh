...
#
# Stages a build context (the 10 canonical CUDA host bins, each embedding its deterministic guest ELF, +
...
echo "=== verify host_tier baked in ==="
docker exec zkorage-worker-gpu sh -c 'ls -la /prover/target/release/host_tier && echo HOST_TIER_BIN=$HOST_TIER_BIN'
echo "=== DONE $(date +%T) ==="
