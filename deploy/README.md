...
Most app changes are frontend-only. The badge SHA is stamped at `npm run build` on the Windows box, so
build there, then ship `dist` and rebuild just the `frontend` container (backend + keepers untouched):
```bash
